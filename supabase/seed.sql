...
--            27 supplement_items, 33 house_rule_items,
--            22 prohibition_items, 4 original_items,
...
-- --------------------------------------------------------
-- house_rule_items (33 rows)
-- --------------------------------------------------------
INSERT INTO house_rule_items (id, rule_type, supplement_id, about, description) VALUES
...
の二つとなります。
必要な時間は「現実時間5日」「ラクシア時間30日」となります。$desc$);
...
※この方法で手に入れたアビスシャードは原則売却する事が出来ません。$desc$);
...
※当OCPでは、総報酬額がキャップ規定を上回ることはございません。$desc$);

INSERT INTO house_rule_items (id, rule_type, supplement_id, about, description) VALUES
(7, 'common', 8, '追加アビスカースについて', 'ダイスbotに入ってないため適応しません。');

INSERT INTO house_rule_items (id, rule_type, supplement_id, about, description) VALUES
(8, 'common', 8, 'アビススキルについて', '採用しません。');

INSERT INTO house_rule_items (id, rule_type, supplement_id, about, description) VALUES
(9, 'battleSkill', 12, '「シャドウステップ」の裁定について', 'シャドウステップの効果を、戦闘中1Rに1度しか適応できないに変更します');
...
『このアイテムを装備している間、聞き込み判定に常に-2の修正が入ります。また、聞き込み判定に入る前に外す等の行動をする場合、隠蔽判定に成功する必要があります。達成値は卓ごとのGMの判断に任せます。』$desc$);

INSERT INTO house_rule_items (id, rule_type, supplement_id, about, description) VALUES
(11, 'item', 1, '多機能○○ベルト等の装飾品を増やすアイテムについて', '「装飾品」を追加で装備可能とする能力は、装飾品として装備することができる「冒険者アイテム」も、その追加した装備枠に装備できるものとします。');

INSERT INTO house_rule_items (id, rule_type, supplement_id, about, description) VALUES
(12, 'item', 4, 'ミハウ式流円闘技流派武器〈ウォースペード〉について', $desc$以下のように裁定します
アックスB &スタッフB→スタッフB、金額3500G+20名誉点、斬撃武器$desc$);

INSERT INTO house_rule_items (id, rule_type, supplement_id, about, description) VALUES
(13, 'item', 4, 'ノーザンファング鉱士削岩闘法流派武器〈ノーザンファングマトック〉について', $desc$以下のように裁定を修正します
ウォーハンマーB&スタッフB→スタッフA$desc$);

INSERT INTO house_rule_items (id, rule_type, supplement_id, about, description) VALUES
(14, 'item', 10, 'トワイライトブランド', '購入段階でダメージボーナスを得る種族を決定してください。一度決定したダメージボーナスを得る種族は変更できません。');

INSERT INTO house_rule_items (id, rule_type, supplement_id, about, description) VALUES
(15, 'item', 10, 'レイジングレイヴ', '【ダークハンター】技能での使用を不可とします。');

INSERT INTO house_rule_items (id, rule_type, supplement_id, about, description) VALUES
(16, 'item', 10, 'コンバットスタッフ', '必筋　8→18');

INSERT INTO house_rule_items (id, rule_type, supplement_id, about, description) VALUES
(17, 'item', 21, '【死骸銃遊戯】の《即応弾》の処理について', 'この弾で発生するダメージは「K10+魔力」の物理ダメージとして処理します');

INSERT INTO house_rule_items (id, rule_type, supplement_id, about, description) VALUES
(18, 'item', 21, '【死骸銃遊戯】の流派武器の処理について', $desc$以下のように裁定します
必筋+3　流派秘伝以外使用時命中-1$desc$);

INSERT INTO house_rule_items (id, rule_type, supplement_id, about, description) VALUES
(19, 'race', 6, 'シザースコーピオンについて', 'レベル6：「装飾品：腰」の装飾品を1つまで「装飾可能部位：その他」に装備可能とします。(ただし、装飾可能アイテムは複数装備してもよい)');

INSERT INTO house_rule_items (id, rule_type, supplement_id, about, description) VALUES
(20, 'race', 6, 'アルボルについて', '種族特徴<炎の盾>は選択できません。');
...
生まれ表https://lite.evernote.com/note/c4c7a0f2-aed8-4b0a-391d-4e2c3611c78b$desc$);

INSERT INTO house_rule_items (id, rule_type, supplement_id, about, description) VALUES
(22, 'skill', 3, '陣率について', '「対象：軍師自身」の効果を軍師のもつすべての部位に適応可能とします。ただし、「1度だけ」等の制約のある陣率は、「部位全体で1度のみ」適応可能とします。');
...
※全ての幻獣は【部位属性付与】および【コア耐久超増強】は取得できず、【部位耐久増強】は意味がありません。また、ムシュフシュとギーブルは【部位即応＆強化】を取得できません。$desc$);

INSERT INTO house_rule_items (id, rule_type, supplement_id, about, description) VALUES
(24, 'skill', 8, 'アビスゲイザーについて', 'ノーザンファング鉱士削岩闘法流派武器〈ノーザンファングマトック〉について');
...
アルボル
ディアボロ
※「アビスゲイザー」「ダークハンター」の条件はPCにかかる条件のため、NPCおよびエネミーには適用されません。つまり、NPCがアビスゲイザーを使用することも、リルドラケンのNPCがダークハンターを使用することができます。$desc$);
...
INSERT INTO house_rule_items (id, rule_type, supplement_id, about, description) VALUES
(27, 'item', NULL, 'マナチャージクリスタルについて', '点数に関わらず、1PCの所持できる数は1個までです。');
...
〇魔力放電【〇〇】
　「部位：魔動砲」での近接攻撃を、行います。この時発生するダメージは【〇〇】属性魔法ダメージとなります。 【〇〇】には購入時に選択した属性の名称が入ります。 また、「部位：魔動砲」で攻撃する際は騎手の主動作を消費します。$desc$);
...
3.荊のローブの装備者への近接攻撃を《かばう》により他キャラクターが引き受けた場合、かばったキャラクターに荊によるダメージは発生しません。<かばう>を行ったキャラクターが、射程：接触で行動していると明記されていないため、<かばう>使用者へ<茨のローブ>によるダメージは発生しないものとします。$desc$);

INSERT INTO house_rule_items (id, rule_type, supplement_id, about, description) VALUES
(30, 'item', NULL, 'バヨネットについて', 'このアイテムは、武器としての加工を不可とします。');
...
・補助動作で使用可能な秘奥魔法の禁止
・「△【コングラーレ=ラピダス】-先手凍結-」はセッション中1度のみ使用可能とします。$desc$);
...
もちろんサプリメントのデータを使用する場合は、対象サプリメントの購入が必要です。$desc$);

-- --------------------------------------------------------
-- prohibition_items (22 rows)
-- --------------------------------------------------------
INSERT INTO prohibition_items (id, about, name) VALUES
(1,  '全般',       '・非売品・取引不可'),
(2,  '特殊',       '・ボトルドール'),
(3,  '装飾品',     $n$・ラル＝ヴェイネの羽冠
・レッサー･アームスフィアⅠ/Ⅱ
...
(10, '<アックスA>', '・紅炎のハルバード'),
(11, '<アックスS>', '・紅炎のクーゼ'),
(12, '<スピアA>',  '・セレスティアルランス ・アイス'),
(13, '<スピアS>',  $n$・アストラルランス
・グレイシャルスピア$n$),
(14, '<スタッフB>', $n$・フレグランススタッフ
・ヘビーフレグランススタッフ$n$),
(15, '<スタッフA>', $n$・愛憎のスタッフ
・フレグランスケイン
・ヘビーフレグランスケイン$n$),
(16, '<フレイルA>', '・バッドツインズ'),
(17, '<ウォーハンマーA>', '・サンダーバッシュ'),
(18, '<ウォーハンマーS>', '・サンダースマッシュ'),
(19, '<ボウA>',    '・スイフトウィンド'),
(20, '騎獣・追加部位', $n$・可変起動外骨格
・信頼の側車$n$),
(21, 'アイテム',   $n$以下の制作者が作成したアイテムの使用を禁止します。
・"万緑"のハヤマ
・"終橙"のハヤミ
・"殺竜鬼"ヴァンダル
・"万眼卿"カルセイ・ライトファーグ$n$),
(22, '装備',       $n$以下の制作者が作成した装備品の使用を禁止します。
・"栄冠与えし者"チャルグ
・"万緑"のハヤマ
・"終橙"のハヤミ
・"殺竜鬼"ヴァンダル
...
SELECT setval('prohibition_items_id_seq',(SELECT MAX(id) FROM prohibition_items));
